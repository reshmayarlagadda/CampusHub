...
import { HiHome, HiCalendar, HiAcademicCap, HiLogout, HiMenu, HiX, HiUser } from 'react-icons/hi';

const navItems = [
  { to: '/organizer', label: 'Dashboard', icon: HiHome, end: true },
  { to: '/organizer/events', label: 'Manage Events', icon: HiCalendar },
  { to: '/organizer/certificates', label: 'Certificates', icon: HiAcademicCap },
];
    ...
    <div className="flex flex-col h-full p-6">
      <div className="mb-6">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl overflow-hidden bg-gradient-to-br from-accent-cyan to-royal-500 flex items-center justify-center font-display font-bold text-white">
            {user?.profileImage ? (
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-slate-500 truncate">{user?.club?.clubName || 'Organizer'}</p>
    ...
    <div className="min-h-screen bg-navy-950">
      ...
      <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
        <div className="lg:hidden flex items-center justify-between px-4 py-3 bg-navy-900/80 border-b border-white/5">
          ...
          <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-accent-cyan to-royal-500 flex items-center justify-center font-display font-bold text-white text-xs">
            {user?.name?.charAt(0)}
          </div>
        </div>
        <main className="flex-1 overflow-y-auto p-4 lg:p-8 bg-mesh"><Outlet /></main>
        <Footer />
      </div>
    </div>
  </div>
